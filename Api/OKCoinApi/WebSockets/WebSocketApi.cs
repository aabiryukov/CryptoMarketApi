...
*/
		...
		private void btc_MessageReceived(object sender, MessageReceivedEventArgs e)
		{
			...
			try
			{
				...
				foreach (var jchannel in jchannels)
				{
					...
					switch (channelName)
					{
						...
						default:
							Log.Error("[OKCoin] Unknown channel name: " + channelName);
							break;
					}
				}
			}
			...
			{
				Log.Error("[OKCoin][Json error] " + exception.Message);
			}
		}
...
//			ThrowMessage("GROUPORDER: " + groupOrder.Market);
		...
		{
			Log.Error("[OKCoin] Websocket ERROR: " + e.Exception.Message);
		}
		...
		private void btc_Opened(object sender, EventArgs e)
		{
			...
			Log.Info("[OKCoin] Websocket opened.");
